...
class WaveSurfaceWithTextsViewKotlin(context: Context?, attrs: AttributeSet?, defStyleAttr: Int) : BaseSurfaceWaveWithTextsViewKotlin(context, attrs, defStyleAttr) {
...
//            val offset = tmpEveryLetterWidths[tmpEveryLetterWidths.size-1] / 2f
    ...
    companion object {
        ...
        const val MAX_SPEED = 80.0
    }

    var mSpeed: Float = 10f
        set(@FloatRange(from = MIN_SPEED,to = MAX_SPEED)value){
            if (value < 1.0 || value > 80.0) {
                throw IllegalArgumentException("Please pass from range 1.0 to 80.0")
            }
            field = value
        }
        get
